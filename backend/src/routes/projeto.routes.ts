...
import { authenticateToken, authorize, authorizeForProject } from '../middleware/auth';
...
const projectSelectorFromParams = (paramKey: string) => (req: express.Request) => {
  const value = (req.params as Record<string, unknown>)[paramKey];
  if (value === undefined || value === null || value === '') {
    throw new Error('Parâmetro de projeto ausente');
  }
...
router.get(
  '/:id',
  authorizeForProject('projetos.ler', { selector: projectSelectorFromParams('id'), requireProject: true }),
...
router.put(
  '/:id',
  authorizeForProject('projetos.editar', { selector: projectSelectorFromParams('id'), requireProject: true }),
  catchAsync(async (req, res): Promise<void> => {
    try {
...
router.delete(
  '/:id',
  authorizeForProject('projetos.excluir', { selector: projectSelectorFromParams('id'), requireProject: true }),
  catchAsync(async (req, res): Promise<void> => {
    try {
